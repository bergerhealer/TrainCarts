...
public class Util {
    ...
    public static void addSignsFromRails(List<Block> rval, Block railsBlock) {
        ...
        if (railType == RailType.NONE) return;
    ...
    public static boolean addAttachedSigns(final Block middle, final Collection<Block> rval) {
        ...
        for (BlockFace face : FaceUtil.AXIS) {
            final Block b = middle.getRelative(face);
    ...
    public static Block findRailsVertical(Block from, BlockFace mode) {
        final int sy = from.getY();
        final int x = from.getX();
        final int z = from.getZ();
        final World world = from.getWorld();
        if (mode == BlockFace.DOWN) {
            for (int y = sy - 1; y > 0; --y) {
                final Block block = world.getBlockAt(x, y, z);
                if (RailType.getType(block) != RailType.NONE) return block;
            }
    ...
    public static String getTimeString(long time) {
        ...
        final StringBuilder rval = new StringBuilder(8);
        ...
        // Hours
        if (hours < 10) rval.append('0');
        rval.append(hours).append(':');

        // Minutes
        if (minutes < 10) rval.append('0');
        rval.append(minutes).append(':');

        // Seconds
        if (seconds < 10) rval.append('0');
        rval.append(seconds);
    ...
    public static BlockFace getPlateDirection(Block plate) {
        ...
        if (s && w) return BlockFace.SELF;
        else if (w) return BlockFace.EAST;
        else if (s) return BlockFace.SOUTH;
    ...
    public static int getOperatorIndex(String text) {
        return IntStream.range(0, text.length()).filter(i -> isOperator(text.charAt(i))).findFirst().orElse(-1);
    }
    ...
    public static boolean matchText(Collection<String> textValues, String expression) {
        return !textValues.isEmpty() && !expression.isEmpty() && (expression.startsWith("!")
                ? !matchText(textValues, expression.substring(1))
                : textValues.stream().anyMatch(text -> matchText(text, expression.split("\\*"),
                expression.startsWith("*"), expression.endsWith("*"))));
    }

    public static boolean matchText(String text, String expression) {
        return !expression.isEmpty() && (expression.startsWith("!")
                ? !matchText(text, expression.substring(1))
                : matchText(text, expression.split("\\*"), expression.startsWith("*"), expression.endsWith("*")));
    }
    ...
    public static boolean evaluate(double value, String text) {
        if (text == null || text.isEmpty()) return false; // no valid input
        final int idx = getOperatorIndex(text);
        if (idx == -1) return value > 0; // no operators, just perform a 'has'

        text = text.substring(idx);
        return text.startsWith(">=") || text.startsWith("=>")
                ? value >= ParseUtil.parseDouble(text.substring(2), 0.0)
                ...
                ? text.startsWith(">")
                ? value > ParseUtil.parseDouble(text.substring(1), 0.0)
                : text.startsWith("<")
                ? value < ParseUtil.parseDouble(text.substring(1), 0.0)
                : text.startsWith("=") && value == ParseUtil.parseDouble(text.substring(1), 0.0)
                : value != ParseUtil.parseDouble(text.substring(2), 0.0);
    }
    ...
    public static boolean canInstantlyBreakMinecart(Entity entity) {
        return TCConfig.instantCreativeDestroy && canInstantlyBuild(entity)
    ...
    public static boolean isSignSupported(Block block, BlockData blockDataOfBlock) {
        final BlockFace face = blockDataOfBlock.getAttachedFace();
    ...
    public static Rails getRailsRO(Block block) {
        final MaterialData data = WorldUtil.getBlockData(block).getMaterialData();
    ...
    public static Vector parseVector(String text, Vector def) {
        final String[] offsetText = splitBySeparator(text);
        final Vector offset = new Vector();
        switch (offsetText.length) {
            case 3:
    ...
    public static double parseAcceleration(String accelerationString, double defaultValue) {
        // Avoid out of range
        if (accelerationString.isEmpty()) return defaultValue;
        ...
        if (slashIndex != -1) {
            ...
            // Process the number value contents before the slash
            {
                ...
                double factor = 1.0;
                final StringBuilder valueStr = new StringBuilder(slashIndex + 1);
                for (int i = 0; i < slashIndex; i++) {
                    final char c = accelerationString.charAt(i);
            ...
            // 20m/s2 -> 20m/ss
            if (num_units == 1) factor *= factor;

            return value / factor;
        }

        // Check whether unit is in g's
        if (accelerationString.charAt(accelerationString.length() - 1) == 'g') {
            // Unit is in g's
            final double value = ParseUtil.parseDouble(accelerationString.substring(0, accelerationString.length() - 1),
                    Double.NaN);
            if (Double.isNaN(value)) return defaultValue;
    ...
    public static double parseVelocity(String velocityString, double defaultValue) {
        final FormattedSpeed speed = FormattedSpeed.parse(velocityString, null);
    ...
    public static double calculateStraightLength(Block railsBlock, BlockFace direction) {
        final TrackWalkingPoint p = new TrackWalkingPoint(railsBlock, direction);
        Vector startDir = null;
        while (p.movedTotal < 20.0 && p.move(0.1)) {
            ...
            if (startDir == null) startDir = p.state.motionVector();
            else if (p.state.position().motDot(startDir) < 0.75) break;
        }
        return p.movedTotal;
...
//        return length;
    ...
    public static int parseTimeTicks(String text) {
        ...
        double ticks = -1.0;
        if (text.endsWith("ms")) ticks = 0.02 * ParseUtil.parseDouble(text.substring(0, text.length() - 2), -1);
        else if (text.endsWith("m")) ticks = 1200.0 * ParseUtil.parseDouble(text.substring(0, text.length() - 1), -1);
        else if (text.endsWith("s")) ticks = 20.0 * ParseUtil.parseDouble(text.substring(0, text.length() - 1), -1);
        else if (text.endsWith("t")) ticks = ParseUtil.parseInt(text.substring(0, text.length() - 1), -1);
    ...
    public static String cleanSignLine(String line) {
        ...
        for (int i = 0; i < line.length(); i++) {
            if (!isInvalidCharacter(line.charAt(i))) continue;
            // One or more character is invalid
            // Proceed to create a new String with these characters removed
            final StringBuilder clear = new StringBuilder(line.length() - 1);
            clear.append(line, 0, i);
            for (int j = i + 1; j < line.length(); j++) {
                final char c = line.charAt(j);
                if (!isInvalidCharacter(c)) clear.append(c);
            }
    ...
    public static String[] cleanSignLines(String[] lines) {
        ...
        if (lines.length != 4) {
            ...
            System.arraycopy(lines, 0, newLines, 0, Math.min(lines.length, 4));
            lines = newLines;
        }
        ...
        for (int i = 0; i < lines.length; i++) {
            final String oldLine = lines[i];
            final String newLine = cleanSignLine(oldLine);
    ...
    public static boolean isProtocolRotationGlitched(float angleOld, float angleNew) {
        return Math.abs(EntityTrackerEntryStateHandle.getProtocolRotation(angleNew)
                - EntityTrackerEntryStateHandle.getProtocolRotation(angleOld)) > 128;
    }
    ...
    public static void spawnDustParticle(Location loc, org.bukkit.Color color) {
        spawnDustParticle(loc, (double) color.getRed() / 255.0, (double) color.getGreen() / 255.0,
    ...
    public static void spawnDustParticle(Location loc, double red, double green, double blue) {
        org.bukkit.Color color = org.bukkit.Color.fromRGB((int) MathUtil.clamp(255.0 * red, 0.0, 255.0),
                (int) MathUtil.clamp(255.0 * green, 0.0, 255.0),
                (int) MathUtil.clamp(255.0 * blue, 0.0, 255.0));
        loc.getWorld().getPlayers().stream().filter(player -> !(player.getLocation().distanceSquared(loc) > (256.0 * 256.0)))
                .forEach(player -> PlayerUtil.spawnDustParticles(player, loc.toVector(), color));
    }
    ...
    public static Location invertRotation(Location loc) {
        ...
        final Quaternion q = Quaternion.fromYawPitchRoll(loc.getPitch(), loc.getYaw(), 0.0);
        q.rotateYFlip();
    ...
    public static BlockFace vecToFace(double dx, double dy, double dz, boolean useSubCardinalDirections) {
        return dy * dy > (dx * dx + dz * dz + 1e-6)
                ? FaceUtil.getVertical(dy)
                : FaceUtil.getDirection(dx, dz, useSubCardinalDirections);
    }
    ...
    public static Vector lerpOrientation(Vector up0, Vector up1, double theta) {
        return Quaternion.slerp(Quaternion.fromLookDirection(up0), Quaternion.fromLookDirection(up1), theta).forwardVector();
    }
    ...
    public static Vector getArmorStandPose(Quaternion rotation) {
        final double qx = rotation.getX();
        final double qy = rotation.getY();
        final double qz = rotation.getZ();
        final double qw = rotation.getW();

        final double rz = 2.0 * (qx * qz - qy * qw);

        // Standard calculation
        if (Math.abs(rz) < (1.0 - 1E-15))
            return new Vector(MathUtil.atan2(2.0 * (qy * qz + qx * qw),
                    1.0 + 2.0 * (-qx * qx - qy * qy)), fastAsin(rz),
                    MathUtil.atan2(-2.0 * (qx * qy + qz * qw), 1.0 + 2.0 * (-qy * qy - qz * qz)));
    ...
    public static Block getNextPos(Block railBlock, BlockFace direction) {
        final TrackMovingPoint p = new TrackMovingPoint(railBlock, direction);
    ...
    public static RailJunction faceToJunction(List<RailJunction> junctions, BlockFace face) {
        return junctions.stream().filter(junc -> junc.position().getMotionFace() == face).findFirst().orElse(null);
    }
    ...
    public static void loadInventoryFromConfig(Inventory inventory, ConfigurationNode config) {
        ...
        if (config.isNode("contents")) {
            final ConfigurationNode contents = config.getNode("contents");
            for (String indexStr : contents.getKeys()) {
                ...
                final ItemStack item = contents.get(indexStr, ItemStack.class);
                if (!ItemUtil.isEmpty(item)) inventory.setItem(index, item.clone());
            }
        }
    }
    ...
    public static void saveInventoryToConfig(Inventory inventory, ConfigurationNode config) {
        ...
        for (int i = 0; i < inventory.getSize(); i++) {
            ...
            if (!ItemUtil.isEmpty(item)) {
                if (contents == null) contents = config.getNode("contents");
    ...
    public static boolean isOrientationInverted(Vector vel, Quaternion q) {
        ...
        double z = q.getZ();
        double w = q.getW();
        return (((vel.getX() * (x * z + y * w) + vel.getY() * (y * z - x * w)
                - vel.getZ() * (x * x + y * y - 0.5))) <= 0.0);
    }
    ...
    public static double fastGetRotationYaw(Quaternion rotation) {
        final double x = rotation.getX();
        final double y = rotation.getY();
        final double z = rotation.getZ();
        final double w = rotation.getW();
        ...
        if (Math.abs(test) < (1.0 - 1E-15)) {
            ...
            // Standard angle
            yaw = MathUtil.atan2(2.0 * (w * y + z * x), 1.0 - 2.0 * (x2 + Math.pow(y, 2)));

            // Wrap around yaw when roll exceeds a limit
            if ((x2 + Math.pow(z, 2)) > 0.5) yaw += (yaw < 0.0) ? 180.0 : -180.0;
        } else {
    ...
    public static void checkMainThread(String what) {
        if (CommonUtil.isMainThread()) return;
    ...
    public static void correctTeleportPosition(Location loc) {
        ...
        final Vector rel = loc.toVector();
        rel.setX(rel.getX() - locBlock.getX());
        rel.setY(rel.getY() - locBlock.getY());
        rel.setZ(rel.getZ() - locBlock.getZ());
        final AxisAlignedBBHandle bounds = WorldUtil.getBlockData(locBlock).getBoundingBox(locBlock);
        if (bounds != null /* AIR */ && rel.getX() >= bounds.getMinX()
                && rel.getX() <= bounds.getMaxX() && rel.getY() >= bounds.getMinY() && rel.getY() <= bounds.getMaxY()
    ...
    public static float getNextEntityYaw(float old_yaw, double yaw_change) {
        ...
        if (yaw_change < -90.0 || yaw_change > 90.0) return old_yaw;

        int protYawRotNew = EntityTrackerEntryStateHandle.getProtocolRotation((float) (old_yaw + yaw_change));
        if (protYawRotNew != EntityTrackerEntryStateHandle.getProtocolRotation(old_yaw)) {
            ...
            float new_yaw = EntityTrackerEntryStateHandle.getRotationFromProtocol(protYawRotNew);
            double new_yaw_change = MathUtil.wrapAngle((double) new_yaw - (double) old_yaw);
            if (yaw_change < 0.0) {
                if (new_yaw_change < yaw_change) {
                    protYawRotNew++;
                    new_yaw = EntityTrackerEntryStateHandle.getRotationFromProtocol(protYawRotNew);
                }
    ...
    public static String stringifyAnimationNodeTime(double time) {
        return time >= 9999
                ? "9999"
                : time >= 999.95
                ? animationodeTime_NumberFormat1000.format(time)
                : time >= 99.995
                ? animationodeTime_NumberFormat100.format(time)
                : time >= 9.9995
                ? animationodeTime_NumberFormat10.format(time)
                : time >= 0.0005
                ? animationodeTime_NumberFormat1.format(time)
                : "0.0";
    }
    ...
    public static boolean isDiagonal(Vector direction) {
        final double sq_x = direction.getX() * direction.getX();
        final double sq_z = direction.getZ() * direction.getZ();
        final double sq_xz = sq_x + sq_z;
        return sq_xz >= 1e-10 && sq_x / sq_xz < SQ_COS_22_5 && sq_z / sq_xz < SQ_COS_22_5;
    }
    ...
    public static boolean isConnectedRails(RailPiece rails, BlockFace direction) {
        // Check not invalid
        if (rails == null || rails.block() == null) return false;
        ...
        final RailJunction junction = Util.faceToJunction(rails.type().getJunctions(rails.block()), direction);
        if (junction == null) return false;
        final RailState state = rails.type().takeJunction(rails.block(), junction);
        if (state == null) return false;

        // Move backwards again from the rails found
        state.setMotionVector(state.motionVector().multiply(-1.0));
        state.initEnterDirection();
        final TrackWalkingPoint wp = new TrackWalkingPoint(state);
        wp.skipFirst();
        return wp.moveFull() && wp.state.railType() == rails.type() &&
                wp.state.railBlock().equals(rails.block());
    }

    public static boolean isUpsideDownRailSupport(Block block) {
        final BlockData blockdata = WorldUtil.getBlockData(block);
        return blockdata != BlockData.AIR
                && (blockdata.isSuffocating(block) || TCConfig.upsideDownSupportedByAll
                && blockdata.canSupportOnFace(block, BlockFace.DOWN));
    }
    ...
    public static Optional<Set<String>> getConfigStringSetOptional(ConfigurationNode config, String key) {
        return config.contains(key)
                ? Optional.of(Collections.unmodifiableSet(new HashSet<>(config.getList(key, String.class))))
                : Optional.empty();
    }

    public static Optional<List<String>> getConfigStringListOptional(ConfigurationNode config, String key) {
        return config.contains(key)
                ? Optional.of(Collections.unmodifiableList(new ArrayList<>(config.getList(key, String.class))))
                : Optional.empty();
    }
    ...
    public static void setConfigStringCollectionOptional(ConfigurationNode config, String key, Optional<? extends Collection<String>> value) {
        ...
        if (value.isPresent()) LogicUtil.synchronizeList(
    ...
    public static <T> Optional<T> getConfigOptional(ConfigurationNode config, String key, Class<T> type) {
        return config.contains(key) ? Optional.ofNullable(config.get(key, type, null)) : Optional.empty();
    }
    ...
    public static void setConfigOptional(ConfigurationNode config, String key, Optional<?> value) {
        if (value.isPresent()) config.set(key, value.get());
            ...
            while (parentYamlPath != YamlPath.ROOT) {
                final String parentPath = parentYamlPath.toString();
                if (!config.isNode(parentPath)) break;
    ...
    public static Player findPlayer(CommandSender sender, String name) {
        if (name.equals("@p")) {
            ...
            final Iterator<Player> iter = axis.world.getPlayers().iterator();
            if (!iter.hasNext()) {
