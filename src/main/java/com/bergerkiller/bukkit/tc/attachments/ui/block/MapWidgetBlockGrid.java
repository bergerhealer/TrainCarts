...
/**
 * Shows a grid of blocks that can be selected using W/A/S/D. Only shows the
 * major Block materials, and the default block state of each.
 */
public abstract class MapWidgetBlockGrid extends MapWidget implements ItemDropTarget, BlockDataSelector {
    ...
    public MapWidgetBlockGrid() {
        ...
        this.calcSize();
        this.addAllBlocks(); // By default show all materials
    }
    ...
    public BlockData getSelectedBlockData() {
        Material block = getSelectedBlock();
        return block == null ? null : BlockData.fromMaterial(block);
    }

    /**
     * Sets the currently selected block material. If the block does not
     * exist in the added items, no item is selected. Does not call
     * {@link #onSelectedBlockDataChanged(BlockData)}!
     * 
     ...
     * 
     * @param itemSize Square resolution (width and height) of the icons
     * @return this item grid widget
     */
    ...
    /**
     * Sets the amount of space between the items in the grid
     *
     * @param itemSpacing Pixel spacing
     * @return this item grid widget
     */
    ...
    /**
     * Clears all previously added blocks so that the grid is now empty
     *
     ...
     * 
     * @param block Block Material to add
     * @return this item grid widget
     */
