...
public class MinecartGroupStore extends ArrayList<MinecartMember<?>> {
    ...
    public static MinecartGroup spawn(Location[] at, EntityType... types) {
        MinecartType[] minecartTypes = new MinecartType[types.length];
        for (int i = 0; i < types.length; i++) {
            minecartTypes[i] = MinecartType.fromEntityType(types[i]);
        }
        return spawn(at, minecartTypes);
    }

    public static MinecartGroup spawn(Location[] at, MinecartType... types) {
